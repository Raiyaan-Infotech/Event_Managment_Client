...
import { Calendar, Users, Share2, Heart, Award, ArrowRight, Star } from "lucide-react"

interface EventInfoScreenProps {
    props: any;
}

export default function EventInfoScreen({ props }: EventInfoScreenProps) {
    ...
    return (
        <div className="w-full h-full relative overflow-hidden rounded-[inherit] transition-all duration-500">
            ...
            <div className="relative z-10 w-full h-full overflow-y-auto chat-scrollbar">
                {/* All event info content is now rendered as movable elements on the canvas */}
            </div>
        </div>
    );
}
